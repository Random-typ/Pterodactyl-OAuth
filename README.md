# Pterodactyl - OAuth
A very basic, more or less secure, OAuth implementation for Pterodactyl Panel. This is not a plugin, but rather a sort of middleware which handles the login.

### ⚠️Pterdactyl Two Factor
...
## Setup
This OAuth implementation works as a sort of middleware. It can be deployed via docker/podman.
### Compose
```
services:
  pterodactyl_oauth:
...
### Redirects
The `/auth/login/*`, `/auth/callback/*` and `/sso/logout/*` path must be redirected to the pterodactyl_oauth container. 
These routes must be bypassed (routed to Pterodactyl) if the `X-Pterodactyl-Route` header is set. 
...
### Optional Redirects
The user could still change their password, email or enable two step verification in Pterodactyl. If you want  
...
You go to your Pterodactyl Panel, and are redirected to `/auth/login/` but still see the normal Pterodactyl  
login screen? This is might be because Pterodactyl routes to `/auth/login/` with js in the client. To fix this
the reload the page. If you navigate to `/auth/login/` manually it will also work.   
To solve this issue code in the Pterodactyl frontend likely needs to be changed.
...
Pterodactyl, once they login again using this script. It also checks wether the user has an admin role  
